...
In Hands-on 1 you briefly used General Refine Expression Language (GREL).  GREL is a powerful and extensible language that helps you manipulate data.  In these next steps we will learn GREL by using practical steps to improve the structure of the data.  
...
7.  Create New Column from Existing Column using a regular expression to match a pattern  
    + INC DATETIME > Edit column > Add column based on this column ...  
        ...
        2. Expression = value.match(/.*\/(\d{4}).*/)[0][^4] > OK  
...
## Normalizing
  ...
  [^4]:  Here’s another example that captures the same data:  `value.match(/(\d\d)\/\d{2}\/(\d{4})\s.*\s([A|P][M])/)[1]`
