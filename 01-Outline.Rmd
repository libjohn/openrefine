...
It has been [written](https://www.nytimes.com/2014/08/18/technology/for-big-data-scientists-hurdle-to-insights-is-janitor-work.html) that 80% of any data project is data cleaning.  And further, 80% of any data cleaning project is tedious while the remaining 20% is  confusing.  

```{r pie1, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}
category1 <- c("Tedious", "Confusing")
category2 <- c("Easy", "Interesting")
portion <- c(80, 20)
before <- as.data.frame(category1, portion)
after <- as.data.frame(category2, portion)

lbls1 <- paste(category1, portion)
lbls1 <- paste(lbls1, "%", sep="")
lbls2 <- paste(category2, portion)
lbls2 <- paste(lbls2, "%", sep="")

pie(portion, labels = lbls1, col = rainbow(length(lbls1)), main="Before OpenRefine")
```

<img src="images/pie1-1.png" alt="Before OpenRefine" class="center" /> 

I prefer to **think of data cleaning as a puzzle** rather than as a tedium.  Good data cleaning is a necessary step that, if given short shrift, will become problematic and still have to be addressed.   But since this is not an analysis book I'll just remind you of the The old adage: "garbage in, garbage out".  To cheer you up, keep in mind that if the bulk of any data project involves data cleaning, you may as well have the best tools at your fingertips and you may as well do a good job.  And you may as well enjoy the puzzle.  

```{r pie2, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}
pie(portion, labels = lbls2, col = rainbow(length(lbls2)), main="After OpenRefine")
```

<img src="images/pie2-1.png" alt="After OpenRefine" class="center" /> 
...
<div class="video"> 
<iframe height="360" width="640" src="https://warpwire.duke.edu/w/C3IBAA/" frameborder="0" scrolling="0" allowfullscreen></iframe>
</div> 
