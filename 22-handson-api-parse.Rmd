...
Now let’s fetch the data from a [Web API](http://www.broadbandmap.gov/developer/) made available via the National Broadband Map ([NBM](http://www.broadbandmap.gov/) | [NBM Developer](https://www.broadbandmap.gov/developer)). This API returns a FIPS code, given a county name (or in this case, even a partial county name.)


2. Fetch JSON data from the National Broadband Map.  We’ll use the [API documentation for Geography by Name API](http://www.broadbandmap.gov/developer/api/census-api-by-geography-name)^[The documentation informs us that the format of the URL we want to construct is as follows:
http://www.broadbandmap.gov/broadbandmap/census/county/durh?format=json ] which returns Census geography for a geography name (e.g. Durham).  In this case we simply use the County `value` as the search string wrapped in the API query protocol.
...
5. Fetch JSON data from the OMDB API
    ...
    + New column name = `JSON data`
    + Throttle delay = `2000`
    + Expression = <br>`'http://www.omdbapi.com/?t=' + escape(value, "url") + '&' + 'apikey=BOOBOOBOY'`
    + Be sure to substitute your actual api key (see above) for `BOOBOOBOY`
    + <span class="or-menu">OK</span>
